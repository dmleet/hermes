...
AlbumTarget
  ...
  lengths in ms, for the sample-rate estimate)
...
MANUAL ─────┘      │
               ...
               IMPORTING ─→ IMPORT_NEEDS_REVIEW (beets skipped; human runs it or retries)
                   ↓
               IMPORTED (terminal; Navidrome scan requested)
...
- One JSON search across every enabled indexer: `GET /api/v1/search?query=<artist> <album>&type=music&categories=3000` (A5). The query is `text.search_form(artist + title)`: MusicBrainz's typographic punctuation (curly quotes, dashes, ellipses) folded to ASCII and apostrophes replaced by spaces, so *Deserter’s Songs* is queried as `Deserter s Songs`. A Gazelle site's Sphinx index splits a title at a typographic apostrophe and blends a straight one, so the pieces are indexed under both spellings while a query carrying either apostrophe matched nothing (measured through Prowlarr on 2026-09-19: 0 results for every apostrophe form and for `Deserters`, all 11 editions for `Deserter Songs` and `Deserter s Songs`). Matching and ranking do the precision work. `search.indexers` (Prowlarr indexer names) restricts the query with `indexerIds`, for a Prowlarr shared with other apps whose audio-capable indexers are not worth Hermes's queries; a name Prowlarr lacks or has disabled is a warning event, and a list that resolves to nothing fails the search (retryable) rather than widening it.
...
Match score (target ↔ title): artist similarity, trying the credit before a "performed by" or "feat." tail; title similarity (rapidfuzz) after stripping an edition suffix, with subtitles handled asymmetrically (MusicBrainz's soundtrack tail or a parenthesised alternative title may be dropped; a subtitle only the tracker has is a different release; when both sides carry one the tails must agree); year within ±1 unless a remaster year explains it.
...
- Post-import: `POST` Navidrome Subsonic `startScan` (optional, config).
