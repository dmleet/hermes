...
- One JSON search across every enabled indexer: `GET /api/v1/search?query=<artist> <album>&type=music&categories=3000` (A5). Matching and ranking do the precision work.
...
StatefulSet/beets (existing, namespace beets; flux repo clusters/home/beets)
  image                      <namespace>/beets-hermes:<version>-<sha>: the pinned
                             lscr.io/linuxserver/beets digest plus the plugin (deploy/beets/Dockerfile)
